...
class MainDialog(QMainWindow):
    ...
    def populate_list_ttl_widget(self):
        ...
        self.ui.tableWidget.setColumnWidth(1, 180)  # 두 번째 열의 너비를 150으로 설정
        ...
        for i in range(len(ranklist)):
            ...
            item_view = QTableWidgetItem(viewlist[i])
            item_view.setTextAlignment(Qt.AlignCenter)
            self.ui.tableWidget.setItem(i, 0, QTableWidgetItem(item_rank))
            self.ui.tableWidget.setItem(i, 1, QTableWidgetItem(item_name))
            self.ui.tableWidget.setItem(i, 2, QTableWidgetItem(item_view))
